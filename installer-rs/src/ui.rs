...
fn draw_welcome(f: &mut Frame, area: Rect) {
    let art = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "  Welcome to the Northstar NixOS Installer",
            Style::default()
                .fg(Color::White)
...
fn draw_done(f: &mut Frame, area: Rect, app: &App) {
    let lines = vec![
        ...
        Line::from(""),
        Line::from(format!(
            "  Configuration saved to: /home/{}/northstar",
            app.config.username
        )),
        Line::from("  You can now reboot into your new Northstar system."),
        Line::from(""),
        Line::from(Span::styled(
            "  After reboot:",
            Style::default()
        ...
        Line::from(Span::styled(
            format!(
                "    cd ~/northstar && sudo nixos-rebuild switch --flake .#{}",
                app.config.hostname
            ),
            Style::default().fg(CYAN),
        )),
