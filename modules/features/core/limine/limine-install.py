...
def install_bootloader() -> None:
    ...
    for profile, gens in profiles:
        gens_sorted = sorted(gens, key=lambda x: x, reverse=True)
        if not gens_sorted:
            continue
            
        # Add current generation at root level
        config_file += generate_config_entry(profile, gens_sorted[0], True)
